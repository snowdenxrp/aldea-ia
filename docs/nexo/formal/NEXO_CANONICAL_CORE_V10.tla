---------------- MODULE NEXO_CANONICAL_CORE_V10 ----------------
...
OpStates == {"PROPOSED","ADMITTED","AUTHORIZED","EXECUTING",
             "EXTERNAL_UNKNOWN","RECONCILED","VERIFIED","COMMITTED"}
...
OperationSet == [state: OpStates, admittedEpoch: Nat]
EffectBindingSet == [operation: Ops, effect: Effects, target: Targets,
                     targetFp: Fingerprints]
AuthoritySet == [epoch: Nat, valid: BOOLEAN]
LeaseSet == [state: LeaseStates, owner: Owners, generation: Nat, expiresAt: Nat]
EvidenceSet == [state: EvidenceStates, operation: Ops, effect: Effects,
                target: Targets, targetFp: Fingerprints,
                observedEffect: EffectStates, authorityEpoch: Nat,
                policyVersion: Nat, graphVersion: Nat, freshUntil: Nat,
                provenance: BOOLEAN]
ReleaseSet == [valid: BOOLEAN, operation: Ops, effect: Effects,
               target: Targets, targetFp: Fingerprints,
               authorityEpoch: Nat, recoveryGeneration: Nat, nonce: Nat]

VARIABLES opState, effectBinding, effectState, authority, policyVersion,
          graphVersion, dependencyState, evidence, recovery,
          reconciliation, execution, stop, stopEpoch, releaseAuth, now

vars == <<opState,effectBinding,effectState,authority,policyVersion,
          graphVersion,dependencyState,evidence,recovery,reconciliation,
          execution,stop,stopEpoch,releaseAuth,now>>

Init ==
  /\ opState = [o1 |-> [state |-> "PROPOSED", admittedEpoch |-> 0]]
  /\ effectBinding = [o1 |-> [operation |-> o1, effect |-> fx1,
  ...
  /\ evidence =
       [e |-> [state |-> "NONE", operation |-> o1, effect |-> fx1,
               target |-> t1, targetFp |-> "fp1",
               observedEffect |-> "UNSEEN", authorityEpoch |-> 0,
               policyVersion |-> 0, graphVersion |-> 0,
               freshUntil |-> 0, provenance |-> FALSE] : e \in EvidenceIds]
  /\ recovery = [o1 |-> [state |-> "FREE", owner |-> alice,
                          generation |-> 0, expiresAt |-> 0]]
  /\ reconciliation = [o1 |-> [state |-> "FREE", owner |-> alice,
                                generation |-> 0, expiresAt |-> 0]]
  /\ execution = [o1 |-> [state |-> "FREE", owner |-> alice,
                           generation |-> 0, expiresAt |-> 0]]
  /\ stop = [o1 |-> "NORMAL"]
  /\ stopEpoch = [o1 |-> 0]
  /\ releaseAuth = [o1 |-> [valid |-> FALSE, operation |-> o1,
                             effect |-> fx1, target |-> t1,
                             targetFp |-> "fp1", authorityEpoch |-> 0,
                             recoveryGeneration |-> 0, nonce |-> 0]]
  /\ now = 0

Admit ==
  /\ opState[o1].state = "PROPOSED"
  /\ authority[o1].valid
  /\ opState' = [opState EXCEPT ![o1] =
...
AcquireExecution(owner) ==
  ...
  /\ execution' = [execution EXCEPT ![o1] =
                      [state |-> "HELD",owner |-> owner,
                       generation |-> @.generation + 1,
                       expiresAt |-> now + 3]]
  /\ opState' = [opState EXCEPT ![o1].state = "AUTHORIZED"]
  /\ UNCHANGED <<effectBinding,effectState,authority,policyVersion,
                 graphVersion,dependencyState,evidence,recovery,
...
AcquireReconciliation(owner) ==
  ...
  /\ reconciliation' = [reconciliation EXCEPT ![o1] =
                           [state |-> "HELD",owner |-> owner,
                            generation |-> @.generation + 1,
                            expiresAt |-> now + 4]]
  /\ UNCHANGED <<opState,effectBinding,effectState,authority,
                 policyVersion,graphVersion,dependencyState,evidence,
                 recovery,execution,stop,stopEpoch,releaseAuth,now>>
...
Observe(eid,owner,generation) ==
  ...
  /\ reconciliation[o1].expiresAt > now
  ...
  /\ UNCHANGED <<opState,effectBinding,effectState,authority,
                 policyVersion,graphVersion,dependencyState,recovery,
                 reconciliation,execution,stop,stopEpoch,releaseAuth,now>>
...
ExpireExecution(owner,generation) ==
  /\ execution[o1].state = "HELD"
  /\ execution[o1].owner = owner
  /\ execution[o1].generation = generation
  /\ execution[o1].expiresAt <= now
  /\ execution' = [execution EXCEPT ![o1].state = "EXPIRED"]
  /\ UNCHANGED <<opState,effectBinding,effectState,authority,policyVersion,
                 graphVersion,dependencyState,evidence,recovery,
                 reconciliation,stop,stopEpoch,releaseAuth,now>>

ExpireReconciliation(owner,generation) ==
  /\ reconciliation[o1].state = "HELD"
  /\ reconciliation[o1].owner = owner
  /\ reconciliation[o1].generation = generation
  /\ reconciliation[o1].expiresAt <= now
  /\ reconciliation' = [reconciliation EXCEPT ![o1].state = "EXPIRED"]
  /\ UNCHANGED <<opState,effectBinding,effectState,authority,policyVersion,
                 graphVersion,dependencyState,evidence,recovery,execution,
                 stop,stopEpoch,releaseAuth,now>>

ExpireRecovery(owner,generation) ==
  /\ recovery[o1].state = "HELD"
  /\ recovery[o1].owner = owner
  /\ recovery[o1].generation = generation
  /\ recovery[o1].expiresAt <= now
  /\ recovery' = [recovery EXCEPT ![o1].state = "EXPIRED"]
  /\ UNCHANGED <<opState,effectBinding,effectState,authority,policyVersion,
                 graphVersion,dependencyState,evidence,reconciliation,
                 execution,stop,stopEpoch,releaseAuth,now>>
...
AuthorizeRelease(owner,generation) ==
  ...
  /\ recovery[o1].generation = generation
  /\ recovery[o1].expiresAt > now
  /\ releaseAuth' = [releaseAuth EXCEPT ![o1] =
...
ChangePolicy ==
  ...
  /\ UNCHANGED <<opState,effectBinding,effectState,authority,graphVersion,
                 dependencyState,recovery,reconciliation,execution,
                 stop,stopEpoch,now>>
...
CompromiseDependency ==
  ...
  /\ evidence' = [evidence EXCEPT ![e].state =
                    ...
                    THEN "INVALIDATED" ELSE evidence[e].state]
  /\ UNCHANGED <<opState,effectBinding,effectState,authority,policyVersion,
                 graphVersion,recovery,reconciliation,execution,stop,
                 stopEpoch,now>>

RevokeAuthority ==
  /\ authority' = [authority EXCEPT ![o1] =
...
Next ==
  ...
  \/ AcquireRecovery(bob)
  \/ ExpireExecution(alice, execution[o1].generation)
  \/ ExpireExecution(bob, execution[o1].generation)
  \/ ExpireReconciliation(alice, reconciliation[o1].generation)
  \/ ExpireReconciliation(bob, reconciliation[o1].generation)
  \/ ExpireRecovery(alice, recovery[o1].generation)
  \/ ExpireRecovery(bob, recovery[o1].generation)
