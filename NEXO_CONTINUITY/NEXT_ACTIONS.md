...
1. Extend the AB55 finite interpreter with the AB56 lower-arity observation and FutureObs_PAA model as executable code, not just prose.
2. Represent histories as EventDAGs with explicit forced/independent/unknown ordering and enumerate legal topological orders.
3. Use explicit UNKNOWN as a third epistemic value and partial-order histories; never invent unspecified successors or concurrency edges.
4. Encode actual UsedAdmissionContext identity/binding equality and protocol-specific continuation legality.
5. Execute the eight highest-value ternary attacks through the complete AB56 gate.
6. Compare FutureObs_PAA sets and deterministic reconstruction for every surviving candidate.
7. Only after the ternary gate is fully specified broaden to all 286 triples.
