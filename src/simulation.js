...
export function tick(simulation, deltaHours = 0.01) {
  ...
  for (const agent of simulation.agents) {
    ...
    try {
      if (!agent.currentIntent && ["drinking", "eating", "fishing", "gathering", "resting"].includes(agent.currentActivity)) agent.currentActivity = "idle";
      ...
      if (criticalFood && agent.needs.hunger <= 70 && (agent.needs.energy > 15 || agent.needs.hunger <= 20)) {
        ...
        agent.currentIntent = { name: "drink", amount: 5, baseValue: 0.5, effects: { thirst: 1.8 }, distance: water.distance, target: { ...simulation.world.resources.water.position } };
        agent.currentActivity = water.distance > 1.5 ? "moving" : "idle";
        agent.decisionSnapshot = { chosen: { name: "drink", score: 999 }, considered: [{ name: "drink", score: 999 }] };
      }
