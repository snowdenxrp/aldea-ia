...
export async function executeNexoStep({
  mission, stepId, adapter, memory=null, context={}, precondition, postcondition
}={}) {
  ...
  const adapterResult=await adapter.execute({
    ...
    idempotencyKey,context:step.context??context,precondition,postcondition
  });
  const outcome=adapterResult.status==="completed" ? "completed"
    : adapterResult.status==="blocked" || adapterResult.status==="unsupported" ? "blocked" : "failed";
  ...
  const nextMemory=recordNexoOutcome(memory,{missionId:started.missionId,stepId,action:step.action,target:step.target,status:outcome,evidence});
...
// If a LUMINA_ACTION step has no caller-supplied verifier, use the action-specific
// verifier derived from the observed canonical action result.
export async function executeLuminaNexoStep({
  simulation, mission, stepId, memory=null, context={}, precondition, postcondition
}={}) {
  ...
  const actionIntent=step?.context?.action??context?.action??null;
  const effectivePostcondition=postcondition??(
    step?.action==="execute_lumina_action" && actionIntent
      ? createLuminaActionPostcondition(simulation,actionIntent,step.target)
      : undefined
  );
