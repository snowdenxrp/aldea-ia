...
const agents=createInitialAgents(),simulation=createSimulation(world,agents),SAVE_KEY="lumina-world-v9";
...
async function loadRemoteIfNeeded(){try{const local=JSON.parse(localStorage.getItem(SAVE_KEY)||"null");const r=await fetch("./world-state.json?ts="+Date.now(),{cache:"no-store"});if(!r.ok)return;const remote=await r.json();if(!validState(remote))return;const localStamp=(Number(local?.day)||0)*24+(Number(local?.hour)||0);const remoteStamp=(Number(remote?.day)||0)*24+(Number(remote?.hour)||0);if(!validState(local)||remoteStamp>localStamp){applyState(remote);save();syncMeshes();centerOnAgents();}}catch{}}
