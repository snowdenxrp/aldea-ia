...
function gatherWood(simulation,agent,amount){const wood=simulation.world.resources.wood;const skill=getSkillLevel(agent,"gather_wood");const efficiency=.6+skill*.8;const gathered=Math.min(amount*efficiency,wood.amount);if(gathered<=0)return{success:false,reason:"no_wood"};wood.amount-=gathered;agent.inventory.push({type:"wood",amount:gathered});agent.needs.energy=Math.max(0,agent.needs.energy-gathered*Math.max(1.2,2-skill));agent.currentActivity="gathering";return{success:true,effect:"wood_gathered",amount:gathered,skillLevel:skill};}
function gatherStone(simulation,agent,amount){const stone=simulation.world.resources.stone;const skill=getSkillLevel(agent,"gather_stone");const efficiency=.55+skill*.85;const gathered=Math.min(amount*efficiency,stone.amount);if(gathered<=0)return{success:false,reason:"no_stone"};stone.amount-=gathered;agent.inventory.push({type:"stone",amount:gathered});agent.needs.energy=Math.max(0,agent.needs.energy-gathered*Math.max(1.5,2.5-skill));agent.currentActivity="gathering";return{success:true,effect:"stone_gathered",amount:gathered,skillLevel:skill};}
